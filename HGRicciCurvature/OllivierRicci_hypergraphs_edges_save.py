...
_base = math.e
_alpha= 0.5 #alpha useless for edge transportation?
_exp_power = 2
_proc = mp.cpu_count()
...
def _get_single_node_neighbors_distributions(node):
    ...
    """
    neighboring_edges = list(_H.dual().edges[node]) #Hyperedges containing node
...
def _distribute_densities(source, target):
    ...
    LG_adj, rindex = _H.edge_adjacency_matrix(s=1,index=True) ##Play both with s and Jaccard index. Compute it once and for all!!!
    LG=nk.nxadapter.nx2nk(nx.Graph(LG_adj))

    d = []
    for src in source_topknbr:
        tmp = []
        for tgt in target_topknbr:
            tmp.append(_source_target_shortest_path(src, tgt, LG))
        d.append(tmp)
    d = np.array(d)
...
@lru_cache(_cache_maxsize)
def _source_target_shortest_path(source, target, LG):
    ...
    length = nk.distance.BidirectionalDijkstra(LG, source, target).run().getDistance()
...
def _compute_ricci_curvature_edges(H: hnx.Hypergraph, G: nx.Graph, hyperedge_weights, weight="weight", edge_list=[],
...
def _compute_ricci_curvature(H: hnx.Hypergraph, G: nx.Graph, weight="weight", **kwargs):
    ...
    **kwargs
    ...
    edge_ricci = _compute_ricci_curvature_edges(H, G, weight=weight, **kwargs)
...
def _compute_ricci_flow(H: hnx.Hypergraph, G: nx.Graph, weight="weight",
    ...
    **kwargs
    ...
    if nx.get_edge_attributes(G, "original_RC"):
        ...
        _compute_ricci_curvature(H, G, weight=weight, **kwargs)
...
class OllivierRicciHypergraphEdges:
    ...
    def __init__(self, H: hnx.Hypergraph, hyperedge_weights, transport_weight='none', weight="weight", alpha=0.5, method="OTDSinkhornMix",
    ...
    def compute_ricci_curvature_edges(self, edge_list=None):
        ...
        output : dict[(int,int), float]
            A dictionary of edge Ricci curvature. E.g.: {(node1, node2): ricciCurvature}.
        """
        return _compute_ricci_curvature_edges(H=self.H, G=self.G, hyperedge_weights=self.hyperedge_weights, weight=self.weight, edge_list=edge_list,
    ...
    def compute_ricci_curvature(self):
        ...
        self.G = _compute_ricci_curvature(H=self.H, G=self.G, hyperedge_weights=self.hyperedge_weights, weight=self.weight,
                                          alpha=self.alpha, method=self.method,
                                          base=self.base, exp_power=self.exp_power,
    ...
    def compute_ricci_flow(self, iterations=10, step=1, delta=1e-4, surgery=(lambda G, *args, **kwargs: G, 100)):
        ...
        """
        self.G = _compute_ricci_flow(H=self.H, G=self.G, hyperedge_weights=self.hyperedge_weights, weight=self.weight,
