...
def _get_single_node_neighbors_distributions(node):
    ...
    neighboring_edges = list(_H_star.edges[node]) #Hyperedges containing node


    # Get sum of distributions from x's all neighbors
    heap_weight_node_pair = []
    for nbr in neighboring_edges:

        #w = _base ** (-_Gk.weight(node, nbr) ** _exp_power)
        #BE SMART HERE ON THE TYPE OF WEIGHTS
        
        w=_hyperedge_weights[nbr]
    ...
    if not neighboring_edges:
        print('PROBLEM!!')
        # No neighbor, all mass stay at node, shouldn't happen
        return [1], [node]

    if nbr_edge_weight_sum > EPSILON:
        # Sum need to be not too small to prevent divided by zero
        distributions = [ w / nbr_edge_weight_sum for w, _ in heap_weight_node_pair]
    else:
        ...
        distributions = [1 / len(heap_weight_node_pair)] * len(heap_weight_node_pair)
...
def _distribute_densities(source, target):
    ...
    # construct the cost dictionary from x to y, using line graph
    t0 = time.time()
...
def _source_target_shortest_path(source, target):
    ...
    ----------
    source : int
        Source node index in LG
    target : int
        Target node index in LG
    LG : networkit graph
        Line graph
    ...
    #length = len(nx.dijkstra_path(LG, source, target))-1    
    length = nk.distance.BidirectionalDijkstra(_LG, _nx2nk[source], _nx2nk[target]).run().getDistance()
...
def _compute_ricci_curvature_single_edge(source, target):
    ...
    if _G.get_edge_data(source, target)['weight'] < EPSILON:
    ...
    result = 1 - (m / _G.get_edge_data(source, target)['weight'])  # Divided by the length of d(i, j)
...
def _compute_ricci_curvature_edges(H: hnx.Hypergraph, H_star: hnx.Hypergraph, G: nx.Graph, LG:nx.Graph, nx2nk, hyperedge_weights, weight="weight", edge_list=[],
    ...
    _LG = LG
    _H_star = H_star
    _hyperedge_weights = hyperedge_weights
    _weight = weight
    _method = method
    _base = base
...
def _compute_ricci_curvature(H: hnx.Hypergraph, H_star:hnx.Hypergraph, G: nx.Graph, LG:nk.Graph, nx2nk, hyperedge_weights, weight="weight", **kwargs):
    ...
    **kwargs
    ...
    edge_ricci = _compute_ricci_curvature_edges(H, H_star, G, LG, nx2nk, hyperedge_weights, weight='weight', **kwargs)
...
def pairs_to_hyperedge(H:hnx.Hypergraph, G: nx.Graph, hyperedge_weights):
    N_edges = len(hyperedge_weights)
    hyp_curvs = np.zeros(N_edges)
    for k in range(N_edges):
        if len(H.edges[k])==1:
            hyp_curvs[k]=hyperedge_weights[k] #single hyperedges won't be cut
        else:
            hyp_edge = H.edges[k]
            curv_per_hyp=[]
            ...
            hyp_curvs[k]=np.max(curv_per_hyp) #Default with max, try with average too
    return hyp_curvs


def _compute_ricci_flow(H: hnx.Hypergraph, H_star:hnx.Hypergraph, G: nx.Graph, LG: nk.Graph, hyperedge_weights, nx2nk, weight="weight",
    ...
    **kwargs
    ...
    if nx.get_edge_attributes(G, "original_RC"):
        ...
        _compute_ricci_curvature(H, H_star, G, LG, nx2nk, hyperedge_weights, weight='weight', **kwargs)
    ...
    for i in range(iterations):
        ...
        hyperedge_weights = pairs_to_hyperedge(H, G, hyperedge_weights)
        _compute_ricci_curvature(H, H_star, G, LG, nx2nk, hyperedge_weights=hyperedge_weights, weight='weight', **kwargs)
...
class OllivierRicciHypergraphEdges:
    ...
    def __init__(self, H: hnx.Hypergraph, hyperedge_weights, transport_weight='none', weight="weight", s=1, method="OTDSinkhornMix",
        ...
        Parameters
        ----------
        H : HyperNetworkX hypergraph
            A given directional or undirectional HyperNetworkX hypergraph.
        
        transport_weight :
            weights, Jaccard, hyperedge length, s-intersection...
        ...
        self.G =self.hypergraph_to_graph()
        self.s=s
        self.weight = weight
        self.method = method
        self.base = base
        ...
        self.LG, self.nx2nk = self.get_linegraph()
    ...
    def hypergraph_to_graph(self):
        if self.transport_weight == 'none':
            Adj = self.H.adjacency_matrix().todense()
            
        if self.transport_weight == 'edge_length':
            N_nodes = len(list(self.H.nodes()))
            Adj=np.zeros((N_nodes, N_nodes))
            N_edges = len(self.H.edges)
            for h in range(N_edges):
                edge = self.H.edges[h]
                for d in list(itertools.combinations(edge, 2)):
                    Adj[d[0], d[1]]+=1/len(edge)
                    Adj[d[1], d[0]]+=1/len(edge)
        if self.transport_weight == 'jaccard':
            N_nodes = len(list(self.H.nodes))
            Adj = np.zeros((N_nodes, N_nodes))
            H_dual = self.H.dual()
            N_dual_edges = len(list(H_dual.edges()))
            for vi_star in range(1, N_dual_edges+1):
               for vj_star in range(vi_star+1, N_dual_edges+1):
                    ei_star = H_dual.incidence_dict[vi_star-1]
                    ej_star = H_dual.incidence_dict[vj_star-1]
                    intersection_size = len(set(ei_star) & set(ej_star))
            
                    union_size = len(set(ei_star) | set(ej_star))
                    jaccard_index = intersection_size / (union_size)
                    Adj[vi_star-1, vj_star-1] = jaccard_index
                    
            np.fill_diagonal(Adj, 0)
            Adj = np.maximum(Adj, Adj.transpose())
            Adj = np.round((Adj!=0)*(1/(Adj+0.000001)), decimals = 4) #Invert Jaccard weights, distance like
    ...
    def get_linegraph(self):
        if self.transport_weight =='none':
            LG_adj, rindex = self.H.edge_adjacency_matrix(s=self.s,index=True) ##Play both with s and Jaccard index. Compute it once and for all!!!
            LG=nx.Graph(LG_adj)
        
        #With Jaccard index
        if self.transport_weight == 'jaccard':
            N_edges = len(list(self.H.edges()))
            LG_adj=np.zeros((N_edges, N_edges))
            for i in range(N_edges):
                for j in range(i):
                    LG_adj[i,j]=len(set(self.H.edges[i])&set(self.H.edges[j]))/len(set(self.H.edges[i])|set(self.H.edges[j]))
    
            LG_adj=LG_adj+LG_adj.T
            LG_adj = np.round((LG_adj!=0)*(1/(LG_adj+0.000001)), decimals = 4) #Invert Jaccard weights, distance like
            LG=nx.Graph(LG_adj)
        LG_nk = nk.nxadapter.nx2nk(LG, weightAttr='weight')
        
       
        nx2nk_ndict, nk2nx_ndict = {}, {}
        for idx, n in enumerate(LG.nodes()):
            nx2nk_ndict[n] = idx
            nk2nx_ndict[idx] = n
        return LG_nk,  nx2nk_ndict
    ...
    def compute_ricci_curvature_edges(self, edge_list=None):
        ...
        output : dict[(int,int), float]
            A dictionary of edge Ricci curvature. E.g.: {(node1, node2): ricciCurvature}.
        """
        return _compute_ricci_curvature_edges(H=self.H, H_star=self.H_star, G=self.G, LG=self.LG, nx2nk=self.nx2nk, hyperedge_weights=self.hyperedge_weights, weight=self.weight, edge_list=edge_list,
                                              method=self.method,
    ...
    def compute_ricci_curvature(self):
        ...
        self.G = _compute_ricci_curvature(H=self.H, H_star=self.H_star, G=self.G, LG=self.LG, nx2nk = self.nx2nk, hyperedge_weights=self.hyperedge_weights, weight=self.weight,
                                          method=self.method,
                                          base=self.base, exp_power=self.exp_power,
    ...
    def compute_ricci_flow(self, iterations=10, step=1, delta=1e-4, surgery=(lambda G, *args, **kwargs: G, 100)):
        ...
        """
        self.G = _compute_ricci_flow(H=self.H, H_star=self.H_star, G=self.G, LG=self.LG, nx2nk = self.nx2nk, hyperedge_weights=self.hyperedge_weights, weight=self.weight,
